...
n_rainy_days = daily_df['is_rainy'].sum()
n_dry_days = (~daily_df['is_rainy']).sum()
n_cold_days = daily_df['is_cold'].sum()
...
print(f"  Rainy days: {n_rainy_days} ({n_rainy_days/len(daily_df)*100:.1f}%)")
print(f"  Dry days:   {n_dry_days} ({n_dry_days/len(daily_df)*100:.1f}%)")
print(f"  Cold days:  {n_cold_days}")
...
# First compute GLOBAL rainy event fraction as baseline
global_rainy_events = events_geo['day_weather_label'].isin(['rainy', 'rainy_cold']).sum()
global_known_events = events_geo['day_weather_label'].notna().sum()
GLOBAL_RAIN_RATE = global_rainy_events / global_known_events if global_known_events > 0 else 0.5
print(f"  Global safety event rainy rate: {GLOBAL_RAIN_RATE:.3f} ({global_rainy_events}/{global_known_events})")
print(f"  Using this as baseline for enrichment calculation\n")
...
for cid in clusters['cluster_id'].values:
    ...
    # Compare cluster's rainy fraction to GLOBAL rainy fraction of all safety events
    # If cluster matches global rate → weather independent (fixed defect)
    # If cluster is much more rainy than global → weather dependent
    if pct_rainy is not None and n_known >= 3:
        if GLOBAL_RAIN_RATE > 0 and GLOBAL_RAIN_RATE < 1:
            rain_enrichment = pct_rainy / GLOBAL_RAIN_RATE
...
w(f"  Method: Rain enrichment ratio (event-level baseline)")
w(f"  Rainy days: {n_rainy_days}/{len(daily_df)} ({n_rainy_days/len(daily_df)*100:.1f}%)")
w(f"  Global safety event rainy rate: {GLOBAL_RAIN_RATE*100:.1f}%")
w(f"  rain_enrichment = cluster_pct_rainy / global_rainy_rate ({GLOBAL_RAIN_RATE:.3f})")
w(f"  independence = max(0, min(1, 2 - rain_enrichment))")
w(f"  Interpretation:")
w(f"    enrichment ≈ 1.0 → matches global pattern → independence = 1.0 (FIXED DEFECT)")
w(f"    enrichment ≈ 2.0 → 2x more rain-biased than average → independence = 0.0")
w(f"    enrichment ≈ 1.5 → moderately more rain-biased → independence = 0.5")
